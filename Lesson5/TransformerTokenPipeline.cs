...
internal static class TransformerTokenPipeline
{
    ...
    public static List<TransformerTrainingExample> BuildTrainingExamples(IEnumerable<List<int>> encodedLines, int contextSize)
    {
        var examples = new List<TransformerTrainingExample>();
        ...
        foreach (var line in encodedLines)
        {
            for (var index = 0; index <= line.Count - contextSize - 1; index++)
            {
                ...
                examples.Add(new TransformerTrainingExample(context, target));
            }
        }
